...
{
    public enum ZombieState { idle, walking, running, attacking, falling, standing };
    ...
    private void OnAttacking(float dist)
    {
        ...
        {
            zombieState = ZombieState.falling;
            StopCoroutine(damagePlayer);
            healPlayer = player.GetComponent<PlayerBehaviour>().StartHealPlayer(player.GetComponent<PlayerBehaviour>().healValue,
                player.GetComponent<PlayerBehaviour>().secondToHeal);
            StartCoroutine(healPlayer);
            StartCoroutine(ZombieStands(5f));
        }
    }

    public IEnumerator ZombieStands(float time)
    {
        yield return new WaitForSeconds(time);

        Debug.Log("Zombie is standing");
